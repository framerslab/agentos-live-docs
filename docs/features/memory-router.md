...
For workloads whose cost/accuracy profile diverges from LongMemEval-S, see [Adaptive Memory Router](/features/adaptive-memory-router) — derives the routing table from your own calibration data instead of relying on Phase B presets.
...
At gpt-4o reader, dropping the OM-v11 routing produces a +1.0 pp aggregate lift (SSP gains 13.4 pp on canonical, MS loses 4 pp, case-weighted aggregate favors canonical). At gpt-5-mini reader (via [ReaderRouter](/features/read-router)), OM-v11 routing for MS/SSP is statistically tied with canonical, but OM-v11 imposes a 60-120 second observer pipeline per OM-routed case (p95 latency 111 sec with the routing on, 7 sec with it off, a 15× tail-latency reduction by dropping it).

For new sem-embed deployments, the recommended config is **canonical-hybrid for all categories + [ReaderRouter](/features/read-router) per-category reader-tier dispatch + `text-embedding-3-small` embedder**. This is the validated 85.6% headline. The `minimize-cost` preset's table will be re-derived from sem-embed Phase B data in v2.
...
- [Cognitive Pipeline](/features/cognitive-pipeline) - the three-stage classifier dispatch this fits inside
- [Query Router](/features/query-routing) - Stage 1, the memory-or-not gate
- [Reader Router](/features/read-router) - Stage 3, the reader-tier dispatch
- [Ingest Router](/features/ingest-router) - input stage sibling
- [Read Router](/features/read-router) - read stage sibling
- [Adaptive Memory Router](/features/adaptive-memory-router) - self-calibrating extension
- [Cognitive Memory](/features/cognitive-memory) - the storage substrate canonical-hybrid retrieves from
- [HyDE Retrieval](/features/hyde-retrieval) - alternate retrieval strategy MemoryRouter can dispatch to
