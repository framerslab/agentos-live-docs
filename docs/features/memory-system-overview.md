...
[`IngestRouter`](/features/ingest-router) classifies incoming content into one of six kinds and picks an ingest strategy:
...
[`QueryClassifier`](/features/query-routing) is the novel piece most memory libraries skip. It assigns every incoming query a tier:
...
[`MemoryRouter`](/features/memory-router) decides, per T1+ query, which retrieval architecture handles it. Three backends ship:
...
[`AdaptiveMemoryRouter`](/features/adaptive-memory-router) takes a calibration sample list and derives the routing table from your own workload. Use it when:
...
[`ReaderRouter`](/features/read-router#reader-router--reader-model-selection) reuses Stage 2's category classification (zero extra LLM calls) to dispatch the answer call to the best reader for that category. Calibrated from per-category Phase B accuracies:
...
[`ReadRouter`](/features/read-router#read-router--read-strategy-selection) classifies a query+evidence pair into a read intent and picks a reader strategy:
...
[`ConsolidationLoop`](/features/memory-consolidation) is the analogue of slow-wave sleep: background maintenance that prunes, merges, strengthens, derives, compacts, and reindexes. It runs in 6 (now 7) ordered steps with a boolean mutex preventing concurrent runs.
...
- [Cognitive Memory](/features/cognitive-memory) — full primary-source citations for every cognitive-science model used, plus the 8-mechanism implementation reference
- [Memory Operations](/features/memory-operations) — auto-ingest pipeline, agent-facing memory tools, import/export across 6 formats
- [Cognitive Pipeline](/features/cognitive-pipeline) — the classifier-driven dispatch composition
- [Ingest Router](/features/ingest-router), [Memory Router](/features/memory-router), [Read Router](/features/read-router) — per-stage primitives
