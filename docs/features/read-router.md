...
Stage 3 of the [Cognitive Pipeline](/features/cognitive-pipeline). Two sibling primitives, both at the read stage, both classifier-driven, both orthogonal:
...
Sibling stage primitives: [Ingest Router](/features/ingest-router) (input), [Memory Router](/features/memory-router) (recall).
...
Picks the best answer reader per query, dispatched per category, on top of whatever retrieval architecture [MemoryRouter](/features/memory-router) chose.
...
When ReaderRouter is the only classifier-firing primitive in the pipeline (no [MemoryRouter](/features/memory-router), no [QueryClassifier T1+](/features/query-routing) producing a category), it fires its own gpt-5-mini few-shot classifier per query (~$0.0001 / query) to drive the dispatch.
...
| Sem-embed era LongMemEval-S (this benchmark's headline) | yes (canonical-hybrid for all categories) | (Tier 3 minimize-cost preset's MS+SSP → OM-v11 routing was calibrated on CharHash retrieval and is now stale; see [Memory Router](/features/memory-router)) |

---

## Related

- [Cognitive Pipeline](/features/cognitive-pipeline) — composition primitive that wires Read Router + Reader Router into a single pipeline
- [Ingest Router](/features/ingest-router) — input stage sibling
- [Memory Router](/features/memory-router) — recall stage sibling (produces the evidence the read stage consumes)
- [Query Router](/features/query-routing) — Stage 1, the memory-or-not gate (also produces the category that Reader Router consumes)
